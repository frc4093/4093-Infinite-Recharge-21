...
public class Drive extends Subsystem {
    ...
    public static double virtualEncOffsetR;
...
public WPI_TalonFX falconFL;
public WPI_TalonFX falconBL;
private SpeedControllerGroup leftSide;
public WPI_TalonFX falconFR;
public WPI_TalonFX falconBR;
...
private DifferentialDrive drive;
    ...
    private final double ramp = .4;
    private final double allowableCloseLoopError = 50;
...
drive.setMaxOutput(1.0);
    ...
    falconFL.config_kI(0, kI);
    falconFR.config_kI(0, kI);

    falconFL.config_kD(0, kD);
    falconFR.config_kD(0, kD);

    falconFL.configClosedloopRamp(ramp);
    falconFR.configClosedloopRamp(ramp);
    
    falconFL.configAllowableClosedloopError(0, allowableCloseLoopError);
    falconFR.configAllowableClosedloopError(0, allowableCloseLoopError);
...
public void curvature(double xSpeed, double zRotation, boolean q) {
    ...
    public void resetVirtualEnc(){
        ...
        virtualEncOffsetR = falconFR.getSelectedSensorPosition();
    }
    ...
    public void pidDriveStraight(double setPoint){
        // falconFR.set();
        double output =0 ;
        // if (Math.abs(setPoint)<60000){
            //  output = MathUtil.clamp((shortPIDController.calculate(falconFL.getSelectedSensorPosition()-virtualEncOffset,setPoint)),-.5,.5);
        // }else if (Math.abs(setPoint)<100000){
            //  output = MathUtil.clamp((righPIDController.calculate(falconFL.getSelectedSensorPosition()-virtualEncOffset,setPoint)),-.75,.75);
        // }else{
            output = MathUtil.clamp((leftPIDController.calculate(falconFL.getSelectedSensorPosition()-virtualEncOffset,setPoint)),-1,1);
        // }
        arcade(output, 0,false);
        // setOutput(MathUtil.clamp((leftPIDController.calculate(falconFL.getSelectedSensorPosition()-virtualEncOffset,setPoint)),-4,4), 
        // MathUtil.clamp((righPIDController.calculate(-falconFR.getSelectedSensorPosition()-virtualEncOffsetR,setPoint)),-4,4));
        // System.out.println(output);
        Robot.dash.displayData("Output pid", output);
        Robot.dash.displayData("setpoint", setPoint);
    }
    public void pidDriveStraight2(double setPoint){
        
        falconFL.set(ControlMode.Position, setPoint);
        falconFR.set(ControlMode.Position, setPoint);
